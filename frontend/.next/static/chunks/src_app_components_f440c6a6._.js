(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/src_app_components_f440c6a6._.js", {
        ...
        "UploadDocument.useEffect": ()=>{
            if (stemData && wordsRef.current) {
                ...
                Object.entries(stemData.posting_list).forEach({
                    "UploadDocument.useEffect": ([word, docs])=>{
                        const row = postingTbody.append("tr");
                        row.append("td").text(word).style("border", "1px solid #ccc").style("padding", "4px");
                        row.append("td").text(docs.map({
                            "UploadDocument.useEffect": (d)=>`${d.title} (${d.frequency})`
                        }["UploadDocument.useEffect"]).join(", ")).style("border", "1px solid #ccc").style("padding", "4px");
                    }
                }["UploadDocument.useEffect"]);
            }
        }
...
const Search = ()=>{
    ...
    const handleSearch = async ()=>{
        const response = await __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].get(`http://127.0.0.1:8000/api/search/?q=${query}&lang=${language}`);
...
_s(Search, "lnSVDfQYBTGwsHhJSeh4haiT/6s=");
_c = Search;
...
//# sourceMappingURL=src_app_components_f440c6a6._.js.map